...
- (void)addMediaClip:(id)clip timelineObject:(id)timelineObject duration:(double)clipDuration trimStart:(double)trimStart atTimeline:(double)timelinePos into:(NSMutableArray *)clipInfos {
    ...
    // Get the media's timecode origin (unclippedRange.start) for coordinate
    // conversion. CRITICAL: read it from the SAME object whose clippedRange
    // produced trimStart — that is `timelineObject`, not `clip`. For a plain
    // media component the two are identical. But for a synchronized-clip
    // collection, `clip` is the INNER media component (whose unclippedRange is
    // in the source file's embedded time-of-day timecode space, e.g. ~50,348s
    // for DJI footage), while trimStart came from the COLLECTION's clippedRange
    // (zero-based internal timeline, e.g. 25.9s). Subtracting those mismatched
    // spaces yields a large negative number that gets clamped to 0, so every
    // bladed sync-clip piece collapsed to source offset 0 — transcribing the
    // uncut start of the file instead of each clip's real range.
